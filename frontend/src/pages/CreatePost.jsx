    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 md:space-y-12 px-4 md:px-0 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b-4 border-black pb-8">
                <div>
                    <h1 className="text-2xl md:text-6xl font-black text-black tracking-tighter uppercase italic leading-none">New <span className="text-primary-600">Assignment.</span></h1>
                    <p className="text-gray-400 font-bold uppercase tracking-[0.2em] text-[10px] mt-2">Prepare and distribute content to selected members</p>
                </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-0 border border-black bg-white">
                ...
                <div className="p-6 md:p-12 space-y-8 border-b lg:border-b-0 lg:border-r border-black">
                    <div className="space-y-3">
                        ...
                        <input type="text" required placeholder="e.g. Monthly Briefing" className="input-field" value={title} onChange={e => setTitle(e.target.value)} />
                    </div>

                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em]">Instructions & Details</label>
                        <textarea required placeholder="Provide detailed execution steps..." className="input-field min-h-[140px] resize-none" value={description} onChange={e => setDescription(e.target.value)} />
                    </div>

                    {/* Assign members Searchable */}
                    <div className="space-y-3 relative">
                        <div className="flex items-center justify-between border-b border-black/10 pb-2">
                            <label className="text-[10px] font-black uppercase tracking-[0.2em]">Assign Members</label>
                            <span className="text-[10px] text-primary-600 font-bold uppercase">{selectedUsers.length} Selected</span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-8">
                            ...
                            {selectedUsers.length === 0 && (
                                <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest italic py-2">No members assigned yet.</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col bg-gray-50/50">
                    ...
                    >
                        {loading ? <Loader2 className="animate-spin" size={22} /> : (
                            <><span>Publish Assignment</span><ArrowRight size={18} /></>
                        )}
                    </button>
                </div>
            </form>
